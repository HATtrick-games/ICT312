...
int MicroPather::SolveForNearStates( void* startState, std::vector< StateCost >* nearbye, float maxCost )
{
	...
	*/
	...
	for( PathNode* pNode=closedSentinel.next; pNode != &closedSentinel; pNode=pNode->next ) {
		if ( pNode->totalCost <= maxCost ) {
			...
			nearbye->push_back( sc );
		}
	}
#ifdef DEBUG
	for( unsigned i=0; i<nearbye->size(); ++i ) {
		for( unsigned k=i+1; k<nearbye->size(); ++k ) {
			MPASSERT( nearbye->at(i).state != nearbye->at(k).state );
		}
	}
#endif
